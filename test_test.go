...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("[%d] ACTION:DISCONNECT LEADER", leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	DPrintf("[%d] ACTION:CONNECT OLD LEADER", leader1)
	...
	DPrintf("ACTION:NO QUORUM. DISCONNECTED [%d], [%d]", leader2, (leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	DPrintf("ACTION:QUORUM ARISES. Connected [%d]", (leader2 + 1) % servers)
	cfg.checkOneLeader()
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf("[%d] ACTION:DISCONNECT Follower", (leader + 1) % servers)
	...
	DPrintf("[%d] ACTION:RE-CONNECT Follower", (leader + 1) % servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	DPrintf("[%d] ACTION:DISCONNECT Follower", (leader + 2) % servers)
	cfg.disconnect((leader + 3) % servers)
	DPrintf("[%d] ACTION:DISCONNECT Follower", (leader + 3) % servers)
	...
	DPrintf("[%d] ACTION:RE-CONNECT Follower", (leader + 2) % servers)
	cfg.connect((leader + 3) % servers)
	DPrintf("[%d] ACTION:RE-CONNECT Follower", (leader + 3) % servers)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf("[%d] ACTION:Disconnect leader1", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf("[%d] ACTION:Disconnect leader2", leader2)

	// old leader connected again
	cfg.connect(leader1)
	DPrintf("[%d] ACTION:Connect leader1", leader1)

	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	DPrintf("[%d] ACTION:Connect leader2", leader2)
